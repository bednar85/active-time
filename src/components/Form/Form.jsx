...
class Form extends Component {
  ...
  getMatchData(inputText) {
    ...
    while ((currentMatch = regexText.exec(inputText)) !== null) {
      ...
      matchValue = Number(matchValue[0]);
  ...
  getSelectedValues = matchData =>
    matchData
      .filter(match => match.selected === true)
  ...
  highlightText(inputText, matchData) {
    ...
    highlightedText = stringReplaceToArray(inputText, regexText, match => {
      matchIndex++;
      return (
        <Match
          ...
          handleClick={currentComponent.toggleSelectedMatch.bind(
            this,
            matchIndex
          )}
        />
  ...
  updateState(targetKeys) {
    ...
    if (targetKeys.includes('highlightedText')) {
      newState.highlightedText = this.highlightText(
        newState.inputText,
        newState.matchData
      );
  ...
  render() {
    ...
    const rawDisplayText =
      highlightedText.length > 0 ? highlightedText : unescape(inputText);
    ...
    const renderCalculatedTotalTimeArea =
      calculatedTotalTime.length > 0 ? (
        <div className="calculated-total-time">
          This recipe should take about: <em>{calculatedTotalTime}</em>
        </div>
      ) : null;

    const highlightButtonIcon = inputTextChanged ? (
        ...
        <div className="content">
          <div className="instructions">
            <div className="instructions__header">
              <h1>Active Time</h1>
              <h2>
                a tool to find all references to time spent cooking or baking in
                a recipe and then use that information to calculate that
                recipe&apos;s total active time
              </h2>
            </div>
            <ol>
              <li>
                Copy and paste text from a recipe in the textarea on the left.
              </li>
              <li>Click the &quot;Highlight Text&quot; button.</li>
              <li>
                Review all of the highlighted times. The fully yellow ones are
                &quot;selected&quot; and will be included when calculating the
                total time. If you would like to exclude a specific time match
                click it so the yellow background appears more transparent. Time
                matches that are over 3 hours (typically marinades or things
                that need to rest overnight) are &quot;unselected&quot; by
                default.
              </li>
              <li>
                After you&apos;ve reviewed all of the highlighted times and made
                your selections, click the &quot;Calculate Time&quot; button.
              </li>
            </ol>
          </div>
          <div className="legend">
            ...
            <ul>
              <li>
                Yellow - a time match, these are selectable, selecting one means
                that that amount of time will be included when calculating the
                total time
              </li>
              <li>
                Red - almost a time match, this is text that includes a time
                related word but it wasn&apos;t able to find any numbers, these
                are not selectable, if you would like one to be selectable so it
                can be used in calculating the total time edit the input text on
                the left so it includes numbers (i.e. &quot;30 minutes&quot;
                instead of &quot;thirty minutes&quot;), click the
                &quot;Highlight Text&quot; button when you are finished editing
              </li>
              <li>
                Green - a time or recipe related keyword, these are not
                selectable, they are meant to act as helpers as you read the
                recipe
              </li>
            </ul>
          </div>
          ...
          <div className="grid">
            <div className="grid-item">
              ...
              <div className="actions">
                ...
                <button
                  className={classNames('ui', 'button', {
                    positive: selectedValuesChanged
                  })}
                  disabled={matchData.length === 0}
                  onClick={this.calculateTotalTime.bind(
                    this,
            ...
            <div className="grid-item">
              <div className="parsed-display-text">{parsedDisplayText}</div>
            </div>
          </div>
        </div>
      </div>
