...
const advancedArrayReplace = (inputArray, regex, classes) => {
	...
	inputArray.forEach((arrayItem, index) => {
		...
		if(typeof arrayItem === 'string') {
			...
			const parsedStringAsArray = advancedStringReplace(
				arrayItem,
				regex,
...
class App extends Component {
	...
	render() {
		...
		let newlineIndex = -1
		const parsedDisplayText = advancedStringReplace(
			rawDisplayText,
			/<br>/g,
			() => {
				newlineIndex++
					...
					<div className="grid">
						<div className="grid-item">
							...
							<div className="actions">
								<button
									className={cx('ui', 'button', { 'positive': inputTextChanged })}
								...
								<button
									className={cx('ui', 'button', { 'positive': selectedValuesChanged })}
...
class Match extends Component {
	...
	render() {
		...
		return (
			<mark className={cx('time-match', 'selectable', { 'selectable--selected': selected })} onClick={handleClick}>{text}</mark>
		)
	}
}
